...
pub fn parse_equation(input: &str) -> Option<EquationCoefficients> {
    ...
    if input.is_empty() {
        return Some(EquationCoefficients {
            a: 0.0,
            b: 0.0,
            c: 0.0,
        });
            ...
            if let Some(foo) = hashmap.get("names").map(String::as_str) {
                if let Some(total_foo_count) = foo_counts.get(foo) {
                    ...
                    if let Some(bar) = hashmap.get("matrix").map(String::as_str) {
                        if let Some(same_pair_count) =
                            occurrences.get(&(foo.to_string(), bar.to_string()))
                        {
...
#[tauri::command]
pub fn calculate_adjusted_mass(
    masses: Vec<String>,
    mass_error: String,
...
pub fn mass_matcher(
    input_masses: Vec<f64>,
    db_masses: &[f64],
    db_names: Vec<String>,
    db_mnames: Vec<String>,
    db_accessions: Vec<String>,
    db_smiles: Vec<String>,
    db_formulas: Vec<String>,
    db_pos_derivs: Vec<i32>,
    mass_error: String,
    mass_window: String,
    msms_ids: &Vec<String>,
    ...
    for mass in input_masses {
        ...
        for index in low_idx..high_idx {
            ...
            res_map.insert(
                "dMass".to_string(),
                (adj_mass - db_masses[index]).to_string(),
            );
            res_map.insert(
                "dPPM".to_string(),
                (((adj_mass - db_masses[index]) / db_masses[index]) * 1E6).to_string(),
            );
            res_map.insert("names".to_string(), db_name);
            res_map.insert(
                "matrix".to_string(),
                " [".to_string() + &db_mnames[index] + "]",
            );
            ...
            res_map.insert(
                "msms".to_string(),
                msms_exist(msms_ids, db_accessions[index].to_string()),
            );
            res_map.insert("formula".to_string(), db_formulas[index].to_string());
            res_map.insert(
                "possible_derivs".to_string(),
                db_pos_derivs[index].to_string(),
            );
