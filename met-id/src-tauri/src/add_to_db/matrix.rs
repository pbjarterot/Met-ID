...
#[tauri::command]
pub fn add_matrix_to_db_rust(name: String, checkboxes: HashMap<String, bool>, adducts: Vec<String>) -> () {
    ...
    let adducts: Vec<MatrixAdduct> = adducts.chunks(3).map(|chunk| {
        let cnk = chunk.to_vec();
        MatrixAdduct {name: cnk[0].clone(), mz: cnk[1].clone(), derivs: cnk[2].clone()}
    }).collect();
