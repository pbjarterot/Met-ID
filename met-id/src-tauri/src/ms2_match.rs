...
fn bin_spectrum(spectrum: &[(f64, f64)], bin_size: f64) -> Vec<f64> {
    let max_mz = spectrum.iter().map(|(mz, _)| *mz).fold(f64::NAN, f64::max);
    let num_bins = (max_mz / bin_size).ceil() as usize;
    let mut binned_spectrum = vec![0.0; num_bins];
...
pub fn ms2_matcher(query_spectrum: &(Vec<f64>, Vec<i64>), db_spectra: &[(Vec<f64>, Vec<i64>)], binsize: f64) -> Vec<f64> {
    ...
    let binned_spectrum1 = bin_spectrum(&spectrum1, binsize);
    ...
    db_spectra.par_iter().map(|spectrum| {
        ...
        let binned_spectrum2 = bin_spectrum(&spectrum2, binsize);
