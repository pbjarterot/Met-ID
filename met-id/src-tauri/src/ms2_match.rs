...
fn bin_spectrum(spectrum: &[(f64, f64)], bin_size: f64, threshold: f64) -> Vec<f64> {
    ...
    let binned_spectrum = if threshold != 0.0 {
        let max_mz: f64 = spectrum
            .iter()
            .map(|(mz, _)| *mz)
            .filter(|&mz| mz <= threshold)
...
pub fn ms2_matcher(
    query_spectrum: &(Vec<f64>, Vec<i64>),
    db_spectra: &[(Vec<f64>, Vec<i64>)],
    binsize: f64,
    threshold: f64,
) -> Vec<f64> {
    let normalized_query_intensity = normalize_vector(&query_spectrum.1);
    let spectrum1: Vec<(f64, f64)> = query_spectrum
        .0
        .iter()
        .zip(normalized_query_intensity.iter())
    ...
    db_spectra
        ...
        .map(|spectrum| {
            ...
            let spectrum2: Vec<(f64, f64)> = spectrum
                .0
                .iter()
                .zip(normalized_intensity.iter())
                .map(|(&mz, &intensity)| (mz, intensity))
                .collect();
