...
pub fn get_msms() -> Vec<Vec<String>> {
    let conn: r2d2::PooledConnection<SqliteConnectionManager> =
        get_msms_connection("get_msms").unwrap();
    ...
    let db_iter = stmt
        .query_map([], |row: &rusqlite::Row<'_>| {
            ...
            Ok(MS2DbRow {
                ...
                matrix: row.get(7).unwrap_or("".to_string()), //data: row.get(6).unwrap_or(Vec::new()),
            })
        })
        .unwrap();
    ...
    let return_data: Vec<Vec<String>> = vec![
        names,
        identifiers,
        adducts,
        cids,
...
fn get_names_from_identifiers(
    ...
    name_id_conversion.insert("User Input".to_string(), "User Input".to_string());

    Ok(name_id_conversion)
}

pub fn get_msms_spectra(identifier: String, adduct: String, cid: String) -> String {
    let spectra: (Vec<(Vec<f64>, Vec<i64>)>, Vec<String>) =
        get_single_msms_spectra(identifier, adduct, cid);
...
fn get_single_msms_spectra(
    identifier: String,
    adduct: String,
    cid: String,
) -> (Vec<(Vec<f64>, Vec<i64>)>, Vec<String>) {
    let conn: r2d2::PooledConnection<SqliteConnectionManager> =
        get_msms_connection("get_single_msms_spectra").unwrap();
...
fn collect_msms_spectra(binary_data: BinaryData) -> (Vec<f64>, Vec<i64>) {
    ...
    for i in (0..binary_data.binary.len()).step_by(mem::size_of::<f64>() + mem::size_of::<i64>()) {
        ...
        let i64_bytes = &binary_data.binary
            [i + mem::size_of::<f64>()..i + mem::size_of::<f64>() + mem::size_of::<i64>()];
...
pub fn ms2_search_spectra(
    ...
    let conn: r2d2::PooledConnection<SqliteConnectionManager> =
        get_msms_connection("ms2_search_spectra").unwrap();
    let name_condition: &str = &name.as_str();
    let sort_field: &str = "name";

    let mut query: String =
        "SELECT identifier, adduct, cid, window, tof, mz, spectra, name, matrix FROM MSMS "
            .to_string();
    ...
    let db_iter: Box<dyn Iterator<Item = Result<Option<MS2DbRow>, rusqlite::Error>>> =
        if let Ok(fragment_mz) = fragment.parse::<f64>() {
            ...
            Box::new(
                stmt.query_map([], move |row: &rusqlite::Row<'_>| {
                    let spectrum: (Vec<f64>, Vec<i64>) = collect_msms_spectra(BinaryData {
                        binary: row.get(6).unwrap_or(Vec::new()),
                    });
    ...
    let filtered_iter = db_iter.filter_map(|result| match result {
...
fn move_to_front<T: PartialEq>(
    names: &mut Vec<T>,
    identifiers: &mut Vec<T>,
    adducts: &mut Vec<T>,
    cids: &mut Vec<T>,
    windows: &mut Vec<T>,
    tofs: &mut Vec<T>,
...
pub fn match_msms_to_ui(
    ...
    let db_iter = stmt
        .query_map([], |row: &rusqlite::Row<'_>| {
            ...
            let spectrum = collect_msms_spectra(BinaryData {
                binary: row.get(7).unwrap_or(Vec::new()),
            });
            let adduct_final_value = if adduct_value.is_empty() {
