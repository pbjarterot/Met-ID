...
pub fn ms2_search_spectra(name: String, fragment: String, ms1mass: String, fragmentslider: String, _ms1massslider: String) -> Vec<Vec<String>> {
    let conn: r2d2::PooledConnection<SqliteConnectionManager> = get_msms_connection("ms2_search_spectra").unwrap();
    ...
    let db_iter: Box<dyn Iterator<Item = Result<Option<MS2DbRow>, rusqlite::Error>>> = if let Ok(fragment_mz) = fragment.parse::<f64>() {
        ...
        Box::new(stmt.query_map([], move |row: &rusqlite::Row<'_>| {
            let spectrum: (Vec<f64>, Vec<i64>) = collect_msms_spectra(BinaryData { binary: row.get(6).unwrap_or(Vec::new()) });
