...
pub fn get_db_data(
    ...
    let mut query: String =
        "SELECT CAST(metabolites.mz AS REAL) + CAST(adducts.deltamass AS REAL) AS adjusted_mz, "
            .to_string();
    ...
    query += &format!(
        "WHERE adducts.numfunctionalgroups <= 100 AND metabolites.id = {} ORDER BY adjusted_mz",
        index
    );
...
pub fn db_ids_and_names(inputvalue: String) -> Vec<(String, usize, i64)> {
    ...
    let mut stmt: rusqlite::Statement = conn
        .prepare("SELECT name, id FROM metabolites WHERE name LIKE ?1 LIMIT 1000")
